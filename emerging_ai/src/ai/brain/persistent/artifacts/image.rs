use crate::brain::memory::artifacts::image::*;
use std::fs;
use std::path::Path;
use bincode;
...
impl MemoryImage {
    ...
    pub fn load_from_bin(path: &Path) -> std::io::Result<Self> {
        ...
        let deserialized: MemoryImage = bincode_next::decode_from_slice(&data, bincode_next::config::standard()).map(|(v, _)| v)?;
        Ok(deserialized)
    }
    ...
    pub fn load_from_ron(path: &Path) -> std::io::Result<Self> {
        ...
        let deserialized: MemoryImage = from_str(&data)?;
        Ok(deserialized)
    }
}