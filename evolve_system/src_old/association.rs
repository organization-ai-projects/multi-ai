/// Ce fichier implémente un graphe générique (`Graph`) et un index vectoriel (`VectorIndex`).
/// Rôle : Gérer des relations génériques entre des concepts (`Graph`) et effectuer des recherches basées sur des vecteurs (`VectorIndex`).
...
impl VectorIndex {
    ...
    pub fn add(&mut self, id: &str, v: Vec<f32>) {
        self.embeddings.insert(id.to_string(), v);
    }
...
// Exemple d'utilisation du système d'association
pub fn process_association(raw: &str, shared_graph: &mut Graph, shared_vectors: &mut VectorIndex) {
    // 1. Vectorisation (embeddings bidon ici)
    let emb = encode(raw);

    // 2. Recherche voisins
    if let Some((voisin_id, score)) = shared_vectors.find_similar(&emb) {
        if score < 0.7 {
            // Aucun voisin ou similarité trop faible : NOUVEAU CONCEPT
            shared_graph.add_node(GraphNode {
                id: raw.to_string(),
                label: raw.to_string(),
                data: None,
                neighbors: HashSet::new(),
            });
            shared_vectors.add(raw, emb);
        } else {
            println!("Le plus proche : {voisin_id} avec un score de {score}");
            // Ajouter un lien dans le graph vers le plus proche
            shared_graph.add_edge(raw, &voisin_id);
        }
    } else {
        // Aucun voisin trouvé
        shared_graph.add_node(GraphNode {
