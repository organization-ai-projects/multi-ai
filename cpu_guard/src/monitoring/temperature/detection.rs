...
pub fn get_max_temperature(stat_sys: &StatSystem, ia_name: &str) -> f32 {
    ...
    match temp_result {
        ...
        Err(e) => {
            ...
            if should_print {
                println!("[{}] Impossible de lire la température CPU: {} (messages limités)", ia_name, e);
            }
            ...
            #[cfg(windows)]
            {
                ...
                if let Some(temp) = result {
                    ...
                    if cpu_usage > 80.0 {
                        println!("[{}] AVERTISSEMENT: Charge CPU élevée ({:.1}%) pendant une période prolongée - risque de surchauffe", ia_name, cpu_usage);
